...
3. If the change is UI-relevant (default assumption — only skip if explicitly told otherwise): rebuild and test in the real app.
   ...
   - If `Turbotext` is already running: agents may run `killall Turbotext` autonomously for Turbotext rebuild, installation, and real-app verification on this Mac; then install + start the new build
