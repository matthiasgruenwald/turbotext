...
**Segment-Bergung bei Engine-Fehler** — Schmaler Rettungspfad im Live-Diktat (nur Live-Streaming-Pfad, heute Apple Speech): Stirbt der Stream durch einen Engine-Fehler ohne Zutun der Nutzerin, werden die bereits finalisierten Segmente sofort automatisch eingefügt; der volatile, noch nicht finalisierte Schwanz wird verworfen. Einen Neuversuch-Pfad gibt es nicht — neu versuchen heißt eine neue Aufnahme über den bestehenden Shortcut. Biegt die Regel aus „Laufbezogene Verarbeitungsanzeige" (genau ein Einfügen nach vollständigem Workflow) nur in der Zeitbedingung: „genau ein Einfügen" bleibt, „nach Abschluss des gesamten Workflows" wird zu „bei Fehler, sobald Abschluss unmöglich". Nutzerabbruch bleibt lautlos ohne Bergung. Die Pille wechselt in einen persistenten Fehlerzustand (offen bis aktiv geschlossen) mit Fehler-Indikator, verständlicher Ursache falls bekannt, dem Hinweis „gesicherte Teile eingefügt, volatiler Rest verloren" und der Kennzeichnung, dass der Text ohne die finale, workflow-spezifische Nachbearbeitung ist. Dateibasierte Backends (Groq/OpenAI/WhisperKit) haben keine Live-Segmente; deren Fehler trifft eine intakte Aufnahmedatei und ist ein anderer Fehlerpfad. Siehe [ADR 0005](docs/adr/0005-bergung-bei-engine-fehler.md).
